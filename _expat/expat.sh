...
LIB_TARBALL=$TARBALLDIR/expat-$LIB_VERSION.tar.bz2
LIB_SRC=$SRCDIR/expat-${LIB_VERSION}
...
download()
{
    if [ ! -s $LIB_TARBALL ]; then
        echo "Downloading expat ${LIB_VERSION}"
        curl -L -o $LIB_TARBALL http://sourceforge.net/projects/expat/files/expat/${LIB_VERSION}/expat-${LIB_VERSION}.tar.bz2
    fi
...
unpack()
{
    ...
    echo Unpacking expat into $SRCDIR...
...
buildForIPhoneOS()
{
    ...
    echo Building expat for iPhoneSimulator
    ...
    echo Building expat for iPhone
...
scrunchAllLibsTogetherInOneLibPerPlatform()
{
    ...
    $ARM_DEV_CMD lipo "iphone-build/lib/expat.a" -thin armv7 -o $IOSBUILDDIR/armv7/expat.a
    $ARM_DEV_CMD lipo "iphone-build/lib/expat.a" -thin armv7s -o $IOSBUILDDIR/armv7s/expat.a
    $ARM_DEV_CMD lipo "iphone-build/lib/expat.a" -thin arm64 -o $IOSBUILDDIR/arm64/expat.a

    $SIM_DEV_CMD lipo "iphonesim-build/lib/expat.a" -thin i386 -o $IOSBUILDDIR/i386/expat.a
    $SIM_DEV_CMD lipo "iphonesim-build/lib/expat.a" -thin x86_64 -o $IOSBUILDDIR/x86_64/expat.a
}
...
buildFramework()
{
    ...
    $ARM_DEV_CMD lipo -create $BUILDDIR/*/expat.a -o "$FRAMEWORK_INSTALL_NAME" || abort "Lipo $1 failed"

    echo "Framework: Copying includes..."
    cp -r $PREFIXDIR/include/expat/*  $FRAMEWORK_BUNDLE/Headers/
